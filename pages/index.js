...
// ── Call our own server (which holds the key) ────────────────────────────────
...
// ── PPTX builder (client-side, uses pptxgenjs from CDN) ─────────────────────
...
export default function Home() {
  ...
  const handleFile = async (e) => {
    ...
    try {
      ...
      // Extract structure
      addLog("🔵 Sending to Gemini to extract structure…");
      const rawExtract = await callGemini(EXTRACT_PROMPT, base64);
      const structure = parseJSON(rawExtract);
      addLog(`✅ Found ${structure.slides?.length || 0} sections — "${structure.title}"`);
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="Turn your BTEC assignment PDF into a paraphrased PowerPoint presentation" />
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ textAlign: "center", marginBottom: "2rem" }}>
            ...
            <h1 style={{ margin: 0, fontSize: 26, fontWeight: 700, color: "#f0f4fa" }}>
              BTEC → PowerPoint
            </h1>
          ...
          {!busy && step !== STEPS.DONE && (
            <label style={{
              ...
              onMouseLeave={e => { e.currentTarget.style.background = "rgba(59,130,246,0.04)"; e.currentTarget.style.borderColor = "rgba(59,130,246,0.5)"; }}
            >
              <input
                ref={fileRef}
                type="file"
          ...
          {step === STEPS.DONE && (
            <div style={{
              ...
              <div style={{ color: "#4ade80", fontSize: 12, marginTop: 4 }}>{fileName} — check your downloads folder</div>
              {preview && (
                <div style={{ marginTop: 14, textAlign: "left" }}>
                  <div style={{ color: "#475569", fontSize: 11, marginBottom: 6, textTransform: "uppercase", letterSpacing: 1 }}>Slides generated</div>
